...
class AAPLFile:
    ...
    def write_files(self, year):  # IO bound operation
    ...
    def calc_averages(self, file):  # CPU bound operation
        low_sum = open_sum = vol_sum = high_sum = close_sum = adj_sum = 0
        year = file.split('_')[1].split('.')[0]
        full_file_path = os.path.join(self.valid_dir(), file)
...
if __name__ == '__main__':
    ...
    all_files = sorted(list(os.listdir(og.valid_dir())))
    all_years = og.get_years_list()
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        executor.map(og.write_files, all_years)
        executor.map(og.calc_averages, all_files)
