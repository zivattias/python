...
select
	avg(income) as avg_income_for_complaining_custs
from
	superstore_data
where
	complain != 0;
...
from
	superstore_data
where
	date_part('year', now()) - year_birth between 20 and 40
order by
	date_part('year', now()) - year_birth desc;
